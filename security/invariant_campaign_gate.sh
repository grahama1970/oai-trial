...
echo "== native DOCX release-gate campaign =="
DOCX_RUN_ROOT="$SEC/runs/docx-latest"
docker build --build-arg INCLUDE_MULTIMODAL=1 -t "$IMAGE" "$REPO" >/dev/null
docker run --rm -v "$SEC/runs:/w" --entrypoint rm "$IMAGE" -rf /w/docx-latest >/dev/null 2>&1 || true
mkdir -p "$DOCX_RUN_ROOT"
python3 - "$BATTLE" "$DOCX_RUN_ROOT" "$IMAGE" "$(id -u)" "$(id -g)" <<'PYDOCX'
import json, sys
from pathlib import Path
battle, run_root, image, uid, gid = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5]
request = json.loads(Path("security/battle/request.template.json").read_text())
request["arena_protocol_path"] = str(Path("security/battle/docx.arena.json").resolve())
request["profile_path"] = str(Path("security/battle/docx.profile.json").resolve())
request["generator"] = str(Path("security/battle/docx_generator.py").resolve())
request["judge"] = str(Path("security/battle/docx_judge.py").resolve())
request["functional_judge"] = str(Path("security/battle/docx_judge.py").resolve())
request["target_run_cmd"] = (
    f"docker run --rm --user {uid}:{gid} "
    "-v {input}:/trial/input:ro -v {output}:/trial/output "
    f"{image} redact-document --input /trial/input/source.docx --policy /trial/input/policy.json "
    "--output /trial/output/out.docx --receipt /trial/output/receipt.json"
)
runtime_lock = json.loads(Path(request["lock_path"]).read_text())
runtime_lock["source_repo_path"] = battle
runtime_lock_path = Path(run_root, "battle.runtime-lock.json")
