...
| R2 | Replace every policy-identified value | `matcher.py` | seeded values absent from output | value split across chunk still replaced | verifier counts source occurrences vs output replacements (#8) |
...
| R6 | Preserve non-sensitive meaning/structure | adapters | non-sensitive cells/keys/rows unchanged | — | verifier structural fingerprint parity |
| R7 | Preserve CSV headers/rows, JSON structure, SQLite tables/relationships/row-counts/integrity | `formats/*` | header/order/rowcount preserved | sensitive-in-header/key/identifier → reject (sem #9) | `integrity_check`+`foreign_key_check`+rowcount+schema hash |
...
| R9 | Keep raw inputs, mappings, quarantine out of release & logs | `pipeline.py` (#4) | release has only corpus+report | mapping/sidecar/tmp in release → fail | verifier rejects extra artifacts |
...
| R11 | Encoding policy (UTF-8/BOM/malformed) | text/csv/json adapters (sem #6,#8) | BOM preserved | malformed UTF-8 rejected, no excerpt | verifier confirms BOM flag + strict decode |
